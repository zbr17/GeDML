...
class CalculatorFromMat:
    ...
    Args:
        ...
        is_min (bool):
            Whether to choose the min (or max) value as the "nearest" sample.
    ...
    """
    def __init__(self, k_list, include=(), exclude=(), k=None, is_min=True):
    ...
    def get_accuracy(self, metric_mat: np.ndarray, query_labels: np.ndarray, reference_labels: np.ndarray, embeds_same_source: bool, include=(), exclude=(), device_ids=None):
        """
        Compute all evaluation indicators. 

        Args:
            metric_mat (np.ndarray):
                Distance matrix to be tested. size: :math:`B_1 \\times B_2`.
        ...
        if any(x in self.requires_knn() for x in self.get_curr_metrics()):
            ...
            knn_indices, knn_distances = metrics.get_knn_from_mat(metric_mat, num_k, embeds_same_source, is_min=self.is_min, device_ids=device_ids)
